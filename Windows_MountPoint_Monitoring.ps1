...
for($i=0; $i -lt $arg_cnt; $i++)
{
	...
	foreach ($line in $input)
    {
	    #$line
        $drive=($line.Split('=;')[1]);
        $fslabel=($line.Split('=;')[3]) -replace ('\W','');
        $freespace=($line.Split('=')[3]) -replace ".$";

        if ( ($drive -ne "") -and (-not [string]::IsNullOrWhiteSpace($freespace))){
    	"$drive,$freespace" | Out-File -Append $BASE_PATH\final.txt
     	}
	    elseif( ($fslabel -ne "") -and (-not [string]::IsNullOrWhiteSpace($freespace)))
        {
        "$fslabel,$freespace" | Out-File -Append $BASE_PATH\final.txt
	    }
    } ###Genrate Merge Columns 

    $log=get-content $BASE_PATH\final.txt 
    
    foreach ($line in $log)
    { 
        $KPI_NAME=(($line.Split(',')[0])) ;
        $VALUE=(($line.Split(',')[1])); 
         xmlTmpWrite ($KPI_NAME) ($VALUE);
	} ### Create Final KPI with Value.
